...
The second three bits for user who created that, next three for group of users and last one is for all users.
...
if we use sh then the default shell interpreter will execute that script but using ./ will use the interpreter that is used in that script for this we need to enable the permissions.
